...
@interface toTheSceneAddPhotoView ()
...
@implementation toTheSceneAddPhotoView
...
- (instancetype)initWithCoder:(NSCoder *)coder
{
    ...
    if (self) {
        ...
        if (!self.mediaView) {
            ...
            self.mediaView = [[UICustomCollectionView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, Cell_Height) collectionViewLayout:layout];
...
//    self.mediaView.frame = CGRectMake(0, 43, [UIScreen mainScreen].bounds.size.width, [toTheSceneAddPhotoView cellHeightWithObj:_curTweet.count] - 35);
    //    [self.mediaView setHeight:[PersonDataAddPicCell cellHeightWithObj:_curTweet.count] - 35];
    [_mediaView reloadData];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row == _curTweet.count) {
        ...
        if (_curTweet.count == 3) {
            
            [YJProgressHUD showMessage:@"不能再添加图片" inView:[UIApplication sharedApplication].keyWindow afterDelayTime:2];
            return;
        }
