...
- (void)beginLoading{
    ...
    if (!self.loadingView) { //初始化LoadingView
        self.loadingView = [[EaseLoadingView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, self.frame.size.height == 0 ? [UIScreen mainScreen].bounds.size.height - 64 : self.frame.size.height )];
...
//            [_reloadButton setTitleColor:RGB_COLOR(86, 86, 86) forState:UIControlStateNormal];
