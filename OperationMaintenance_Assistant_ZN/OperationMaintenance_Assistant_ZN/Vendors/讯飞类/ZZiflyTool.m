...
- (void)onResult:(NSArray *)resultArray isLast:(BOOL)isLast
{
    ...
    if ([self.delegate respondsToSelector:@selector(speakRecognizerError:result:)]) {
        
        [self.delegate speakRecognizerError:nil result:resultFromJson];
...
- (void) onError:(IFlySpeechError *) error{
    ...
    if (error.errorCode!=0) {
        ...
        if ([self.delegate respondsToSelector:@selector(speakRecognizerError:result:)]) {
            ...
            [self.delegate speakRecognizerError:error result:nil];
